...
public class DataBaseQueries {
    ...
    /***Fonction qu irecuperer les statistiques des courriers traités par mois***/
    public static void recupererStatistiquesSurTousLesCourriersParMoisParDirection(String nomDirection){
        ...
        ResultSet resultSetRecus = null;
        ResultSet resultSetEnvoyes = null;

        try {
            resultSetRecus = connectionCourrierRecus.createStatement().executeQuery(requeteCourrierRecus);
            resultSetEnvoyes = connectionCourrierEnvoyes.createStatement().executeQuery(requeteCourrierEnvoyes);
            ...
            while (resultSetRecus.next()){
                listeCourriersRecus.add(resultSetRecus.getString("courrier.date_enregistrement"));
            }

            while (resultSetEnvoyes.next()){
                listeCourriersEnvoyes.add(resultSetEnvoyes.getString("courrier.date_enregistrement"));
            }
            ...
            for(int a = 0; a < finalList.size(); a++){
                try {
                    if(finalList.get(a) != null){
                        switch (DateUtils.recupererNomDuMoisParUneDate(finalList.get(a))){
                            case "janvier":
    ...
    /***Recuperer la direction de l'émeteur du courrier***/
    ...
    public static List<Courrier> recupererTousLesCourriersReçusParUnUtilisateurParSonId(String idUtilisateur) {
        ...
        requeteMesCourriersSQL = "select * from `recevoir_courrier` inner join `courrier` on recevoir_courrier.id_courrier = courrier.id_courrier left join correspondance_dossier_courrier\n" +
                " on correspondance_dossier_courrier.id_courrier = courrier.id_courrier left join dossier on correspondance_dossier_courrier.id_dossier = dossier.id_dossier  inner join personne on recevoir_courrier.id_personne = personne.id_personne inner join direction on personne.id_direction = direction.id_direction where direction.id_direction = '"+idDirection+"' and recevoir_courrier.archive = '"+ EtatCourrier.archiveNonActive +"' and recevoir_courrier.favoris = '"+EtatCourrier.pasfavoris+"' and etat = '"+EtatCourrier.courrierEnvoye+"' group by courrier.id_courrier order by courrier.id_courrier desc";
        ResultSet resultSet = null;
        try {
